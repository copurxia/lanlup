...
compile_error!("nhlogin-rs requires wasm32-wasip1 (target_os = \"wasi\")");

const USER_AGENT: &str =
    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36";
const VALIDATE_URL: &str = "https://nhentai.net/";
...
fn execute_plugin(params: Value) -> Value {
    ...
    let cf_clearance = read_string_param(&params, "cf_clearance");
    let csrftoken = read_string_param(&params, "csrftoken");

    let result = do_login(&cf_clearance, &csrftoken);
...
fn do_login(cf_clearance: &str, csrftoken: &str) -> Result<Value, String> {
    ...
    let mut cookies = vec![LoginCookie {
        ...
        value: cf_clearance.trim().to_string(),
        domain: "nhentai.net".to_string(),
        path: "/".to_string(),
    }];

    if !csrftoken.trim().is_empty() {
        cookies.push(LoginCookie {
            name: "csrftoken".to_string(),
            value: csrftoken.trim().to_string(),
            domain: "nhentai.net".to_string(),
            path: "/".to_string(),
        });
    }
...
fn validate_cookies(cookies: &[LoginCookie]) -> Result<(), String> {
    ...
    if !(200..300).contains(&response.status) {
        return Err(format!("HTTP {}: Failed to access nhentai", response.status));
    }
    ...
    if body_lc.contains("just a moment") || body_lc.contains("checking your browser") {
        return Err("Cloudflare protection detected. Please update your cf_clearance cookie.".to_string());
    }
