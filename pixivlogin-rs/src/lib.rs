...
compile_error!("pixivlogin-rs requires wasm32-wasip1 (target_os = \"wasi\")");

const USER_AGENT: &str =
    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36";
const VALIDATE_URL: &str = "https://www.pixiv.net/ajax/user/self?lang=en";
const DEFAULT_TIMEOUT_MS: i32 = 30_000;
const MAX_REDIRECTS: usize = 5;
const AUTH_DATA_KEY: &str = "__lanlu.phase.pixivlogin.data";
...
fn plugin_info_json() -> Value {
    json!({
        ...
        "author": "lrr4cj",
        "version": "1.0",
        "description": "Stores Pixiv login cookies (PHPSESSID) for Pixiv plugins.",
        "parameters": [
...
fn execute_plugin(params: Value) -> Value {
    ...
    let phpsessid = read_string_param(&params, "PHPSESSID");
    let device_token = read_string_param(&params, "device_token");

    let result = do_login(&phpsessid, &device_token);
    ...
    match result {
        Ok(data) => match HostBridge::task_kv_set(AUTH_DATA_KEY, data.clone()) {
            ...
            Ok(false) => json!({ "success": false, "error": "Failed to persist Pixiv auth data to task KV." }),
            Err(e) => json!({ "success": false, "error": format!("Failed to persist Pixiv auth data: {e}") }),
        },
        Err(e) => json!({ "success": false, "error": e }),
    }
}

fn do_login(phpsessid: &str, device_token: &str) -> Result<Value, String> {
    if phpsessid.trim().is_empty() {
        return Ok(json!({
            "phpsessid": "",
            "device_token": "",
            "message": "No credentials provided. Pixiv plugins will run without authenticated cookies."
        }));
    }

    let mut cookies = vec![LoginCookie {
        name: "PHPSESSID".to_string(),
        value: phpsessid.trim().to_string(),
        domain: "www.pixiv.net".to_string(),
        path: "/".to_string(),
    }];
    if !device_token.trim().is_empty() {
        cookies.push(LoginCookie {
            name: "device_token".to_string(),
            value: device_token.trim().to_string(),
            domain: "www.pixiv.net".to_string(),
            path: "/".to_string(),
        });
    }
    ...
    match validate_cookies(&cookies) {
        Ok(_) => Ok(json!({
            ...
            "message": "Successfully configured Pixiv authentication cookies."
        })),
...
fn validate_cookies(cookies: &[LoginCookie]) -> Result<(), String> {
    ...
    if !(200..300).contains(&response.status) {
        return Err(format!(
            "HTTP {}: Failed to access Pixiv ajax (check cookies).",
            response.status
        ));
    }

    let body: Value = serde_json::from_str(&response.body_text)
        .map_err(|e| format!("warn:Could not parse validation response: {e}. Assuming cookies are correct."))?;
    if body
        .get("error")
        .and_then(Value::as_bool)
        .unwrap_or(false)
    {
        let msg = body
            ...
            .unwrap_or("Invalid Pixiv cookies.");
...
fn parse_url(url: &str) -> Result<ParsedUrl, String> {
    ...
    let (host, port) = if let Some((h, p)) = authority.rsplit_once(':') {
        if p.chars().all(|c| c.is_ascii_digit()) {
            (h.to_string(), p.parse::<u16>().map_err(|_| format!("invalid port in URL: {url}"))?)
        } else {
