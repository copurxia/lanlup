...
fn plugin_info_json() -> Value {
    json!({
        ...
        "author": "Lanlu",
        "version": "1.0.0",
        "description": "Login to Baozi Manhua (包子漫画) and store auth data.",
        "parameters": [
            {"name": "domain", "type": "string", "desc": "Domain: bzmgcn.com | baozimhcn.com | webmota.com | kukuc.co | twmanga.com | dinnerku.com", "default_value": "bzmgcn.com"},
            {"name": "language", "type": "string", "desc": "Language: cn (简体) or tw (繁體)", "default_value": "cn"},
            {"name": "cdn_domains", "type": "string", "desc": "CDN domain for images (empty for auto)", "default_value": ""},
            {"name": "image_quality", "type": "string", "desc": "Image quality: /w640 or empty for original", "default_value": "/w640"},
            {"name": "username", "type": "string", "desc": "Baozi username (optional)", "default_value": ""},
            {"name": "password", "type": "string", "desc": "Baozi password (optional)", "default_value": ""}
        ],
        "permissions": [
            "log.write",
            "progress.report",
...
fn execute_plugin(params: Value) -> Value {
    HostBridge::progress(10, "读取 Baozi 配置...");
    let domain = read_string_param(&params, "domain", "bzmgcn.com");
    let language = read_string_param(&params, "language", "cn");
    let cdn_domains = read_string_param(&params, "cdn_domains", "");
    let image_quality = read_string_param(&params, "image_quality", "/w640");
    let username = read_string_param(&params, "username", "");
    let password = read_string_param(&params, "password", "");

    let base_url = format!("https://{}.{}", language, domain);
    let mut tsid = String::new();

    if !username.is_empty() && !password.is_empty() {
        HostBridge::progress(30, "登录中...");
        match do_login(&base_url, &username, &password) {
            Ok(token) => {
                tsid = token;
                HostBridge::log(1, &format!("bzlogin success, tsid len={}", tsid.len()));
            }
            Err(e) => {
                HostBridge::log(2, &format!("bzlogin login failed: {e}"));
                return json!({ "success": false, "error": e });
            }
        }
    } else {
        HostBridge::log(1, "bzlogin no credentials provided, anonymous mode.");
    }
    ...
    HostBridge::progress(80, "保存认证数据...");
    match HostBridge::task_kv_set(AUTH_DATA_KEY, auth_data.clone()) {
        Ok(true) => {
            HostBridge::progress(100, "配置完成");
            json!({ "success": true, "data": auth_data })
        }
        Ok(false) => json!({ "success": false, "error": "Failed to persist auth data to task KV." }),
        Err(e) => json!({ "success": false, "error": format!("Failed to persist auth data: {e}") }),
    }
}

fn do_login(base_url: &str, username: &str, password: &str) -> Result<String, String> {
    let boundary = "----WebKitFormBoundaryFUNUxpOwyUaDop8s";
    let body = format!(
        "------{boundary}\r\nContent-Disposition: form-data; name=\"username\"\r\n\r\n{username}\r\n------{boundary}\r\nContent-Disposition: form-data; name=\"password\"\r\n\r\n{password}\r\n------{boundary}--\r\n"
    );

    let url = format!("{base_url}/api/bui/signin");
    let mut headers = vec![
        ("Content-Type".to_string(), format!("multipart/form-data; boundary={boundary}")),
    ];
    let resp = http_post(&url, &headers, body.as_bytes())?;
    if resp.status != 200 {
        return Err(format!("Login HTTP {}", resp.status));
    }
    let text = String::from_utf8_lossy(&resp.body);
    let parsed: Value = serde_json::from_str(&text).map_err(|e| format!("JSON parse error: {e}"))?;
    let token = parsed
        .get("data")
        ...
        .to_string();
    if token.is_empty() {
        return Err("Login succeeded but token was empty".to_string());
    }
    Ok(token)
}

fn read_string_param(params: &Value, name: &str, default: &str) -> String {
    params.get(name).and_then(Value::as_str).unwrap_or(default).trim().to_string()
}
...
impl Read for HostTcpStream {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> { self.stream.read(buf) }
}
#[cfg(target_arch = "wasm32")]
impl Write for HostTcpStream {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> { self.stream.write(buf) }
...
fn http_post(url: &str, extra_headers: &[(String, String)], body: &[u8]) -> Result<HttpResponse, String> {
    ...
    let raw = if scheme == "https" {
        read_https(stream, host, req.as_bytes(), body)?
    } else {
        let mut s = stream;
        write_all(&mut s, req.as_bytes())?;
        write_all(&mut s, body)?;
        read_all(&mut s)?
    };
    parse_http(&raw)
}
