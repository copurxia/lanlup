...
class EhdbMetadataPlugin extends BasePlugin {
  ...
  getPluginInfo(): PluginInfo {
    return {
      ...
      version: "1.0",
      description:
        "Queries local EHDB PostgreSQL database for gallery metadata. Faster than web scraping.",
      parameters: [
        {
          name: "connection_string",
          type: "string",
          desc:
            "PostgreSQL connection string (e.g., postgres://user:pass@host:port/database?sslmode=disable)",
        },
      ],
      ...
      update_url:
        "https://git.copur.xyz/copur/lanlu/raw/branch/main/plugins/Metadata/Ehdb.ts",
    };
  }
    ...
    try {
      ...
      if (
        ...
        if (match) {
          ...
          await this.logInfo("search:use_oneshot", {
            gID,
        ...
        if (match) {
          ...
          if (srcMatch) {
            ...
            await this.logInfo("search:use_source_tag", {
              gID,
              gToken: `${gToken.slice(0, 6)}…`,
              domain: sourceDomain,
            });
      ...
      if (!gID) {
        ...
        if (searchResult.success) {
          ...
          await this.logInfo("search:lookup_success", {
            gID,
            gToken: `${gToken.slice(0, 6)}…`,
          });
        } else {
          await this.logWarn("search:lookup_failed", {
      ...
      if (!gID) {
        return {
          success: false,
          error: "No matching gallery found in database!",
        };
      }
      ...
      if (!tagsResult.success) {
        await this.logWarn("search:gdata_failed", {
          gID,
      ...
      return {
        success: false,
        error: `Gallery search failed: ${errorMessage}`,
      };
    }
  }
   ...
   */
  private preprocessTitle(
    title: string,
  ): { core: string; keywords: string[]; artist: string } {
    let core = title;
    let artist = "";
    ...
    for (const pattern of artistPatterns) {
      ...
      if (artistMatch) {
        ...
        if (
          !/^\d+$/.test(extractedArtist) &&
          !/^\d{2,4}[-\/]\d{1,2}$/.test(extractedArtist)
        ) {
  ...
  private calculateSimilarity(
    ...
    // 计算关键词匹配
    const inputWords = inputLower.split(/[\s\-_]+/).filter((w) =>
      w.length >= 2
    );
  ...
  private async lookupGalleryByTitleAndTags(
    title: string,
    tags: string,
  ...
  private async searchByFullText(
    ...
    if (hasJapanese || words.length === 0) {
      await this.logInfo("search:skip_fulltext", {
        reason: hasJapanese ? "has_japanese" : "no_words",
      });
      return {
        success: false,
        error: "Skip fulltext search for Japanese text",
      };
    }
  ...
  private selectBestMatch(rows: any[], input: string): PluginResult {
    ...
    if (bestScore < EhdbMetadataPlugin.MIN_SIMILARITY_SCORE) {
      return {
        success: false,
        error:
          `Best match score (${bestScore}) below threshold (${EhdbMetadataPlugin.MIN_SIMILARITY_SCORE})`,
      };
    }
  ...
  private async getTagsFromDatabase(gID: string): Promise<PluginResult> {
    try {
      ...
      const galleryQuery = `
        ...
        WHERE gid = $1
      `;
      const galleryResult = await this.dbClient.queryObject(galleryQuery, [
        parseInt(gID),
      ]);
