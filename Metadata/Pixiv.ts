...
type PixivIllustBody = {
  ...
  // Dates are returned by Pixiv ajax as ISO strings.
  createDate?: string;
  uploadDate?: string;
  updateDate?: string;
  xRestrict?: number;
...
class PixivMetadataPlugin extends BasePlugin {
  ...
  private normalizeToEpochSeconds(value: unknown): string | null {
    ...
    if (typeof value === "bigint") {
      if (value > 1_000_000_000_000n) return String(value / 1000n);
      return String(value);
    }

    if (typeof value === "number") {
      if (!Number.isFinite(value)) return null;
      if (value > 1_000_000_000_000) return String(Math.floor(value / 1000));
  ...
  getPluginInfo(): PluginInfo {
    return {
      ...
      version: "1.0",
      description:
        "Fetches metadata (title/tags/summary) from Pixiv artwork via web ajax.",
      parameters: [
        {
          name: "lang",
          type: "string",
          desc: "Pixiv ajax lang parameter (e.g. en, ja, zh)",
          default_value: "en",
        },
        {
          name: "merge_existing",
          type: "bool",
          desc: "Merge new tags with existing archive tags",
          default_value: "1",
        },
        {
          name: "prefix_id",
          type: "bool",
          desc: "Prefix title with illustId",
          default_value: "0",
        },
        {
          name: "strip_html",
          type: "bool",
          desc: "Strip HTML tags from Pixiv caption",
          default_value: "1",
        },
        {
          name: "include_translations",
          type: "bool",
          desc: "Also add translated tag names when available",
          default_value: "0",
        },
      ],
      oneshot_arg:
        "Pixiv artwork URL or illustId (e.g. https://www.pixiv.net/artworks/12345678)",
      cooldown: 1,
    ...
    try {
      ...
      if (!illustId) {
        this.outputResult({
          success: false,
          error:
            "No Pixiv illustId found. Provide oneshotParam as Pixiv URL/ID, or add a source:https://www.pixiv.net/artworks/<id> tag.",
        });
        return;
      }

      const cookieHeader = this.buildCookieHeaderForPixiv(
        (input.loginCookies || []) as LoginCookie[],
      );
      ...
      const title = prefixId
        ? `${illustId} ${rawTitle}`.trim()
        : (rawTitle || illustId);
      const summary = stripHtml
        ? this.stripHtml(body.illustComment || "")
        : (body.illustComment || "");
  ...
  private extractIllustIdFromSourceTag(existingTags: string): string | null {
    ...
    const m = existingTags.match(
      /source:\s*(https?:\/\/(?:www\.)?pixiv\.net\/(?:[a-z]{2}\/)?artworks\/(\d+))/i,
    );
    if (m) return m[2];
  ...
  private async fetchIllust(
    illustId: string,
    lang: string,
    cookieHeader: string,
  ): Promise<PluginResult> {
    const url = `https://www.pixiv.net/ajax/illust/${illustId}?lang=${
      encodeURIComponent(lang)
    }`;
    const resp = await this.fetchJsonWithRetry<
      PixivAjaxEnvelope<PixivIllustBody>
    >(url, {
      cookieHeader,
    ...
    if (env.error) {
      return {
        success: false,
        error: env.message || "Pixiv ajax returned error.",
      };
    }
    if (!env.body) {
      return { success: false, error: "Pixiv ajax returned empty body." };
    }
  ...
  private async fetchJsonWithRetry<T>(
    ...
    return {
      success: false,
      error: `Request failed after retries: ${lastErr}`,
    };
  }

  private buildTags(
    body: PixivIllustBody,
    opts: { lang: string; includeTranslations: boolean },
  ): string {
    const out: string[] = [];
