...
type ExtractToCacheResponse = { outputPath: string; relativePath: string; entryName: string; archiveId: string };
...
type PixivMetaJson = {
  ...
  xRestrict?: number;
  aiType?: number;
  tags?: Array<{ tag: string; romaji?: string; translation?: Record<string, string> }>;
  createDate?: string;
  uploadDate?: string;
};
...
class PixivMetadataPlugin extends BasePlugin {
  ...
  protected async runPlugin(input: PluginInput): Promise<void> {
    try {
      ...
      // Prefer meta.json (generated by Pixiv download plugin) when available inside the archive.
      const meta = await this.tryLoadMetaJson(input);
      const illustId =
      ...
      // Use meta.json path when present (offline/fast path).
      if (meta && String(meta.illustId || "").trim() === illustId) {
        const body = this.bodyFromMeta(meta, illustId);
  ...
  private bodyFromMeta(meta: PixivMetaJson, illustId: string): PixivIllustBody {
    return {
      illustId,
      illustTitle: String(meta.illustTitle || "").trim(),
      illustComment: String(meta.illustComment || ""),
      userId: String(meta.userId || "").trim(),
      userName: String(meta.userName || "").trim(),
      userAccount: String(meta.userAccount || "").trim(),
      pageCount: Number(meta.pageCount ?? 1) || 1,
      xRestrict: typeof meta.xRestrict === "number" ? meta.xRestrict : undefined,
      aiType: typeof meta.aiType === "number" ? meta.aiType : undefined,
      tags: { tags: (meta.tags || []).map((t) => ({ tag: String(t.tag || "").trim(), translation: t.translation })) },
  ...
  private async tryLoadMetaJson(input: PluginInput): Promise<PixivMetaJson | null> {
    ...
    try {
      ...
      const files = (listing?.files || []) as string[];
      if (!files.length) return null;

      const metaEntry = files.find((f) => f === "meta.json") ||
        files.find((f) => f.toLowerCase() === "meta.json") ||
        files.find((f) => f.toLowerCase().endsWith("/meta.json")) ||
        files.find((f) => f.toLowerCase().endsWith("\\meta.json"));
      ...
      const raw = await Deno.readTextFile(String(extracted.outputPath));
      const meta = JSON.parse(raw) as PixivMetaJson;
      if (!meta || typeof meta !== "object") return null;
      if (!String((meta as any).illustId || "").trim()) return null;
      return meta;
    } catch {
      return null;
    }
